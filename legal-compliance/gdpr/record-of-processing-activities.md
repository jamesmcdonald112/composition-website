...
- **When any third-party widget is added to the site** (Like button, embedded YouTube, Google Maps, Google Analytics, Stripe, advertising pixel, chat widget, third-party font, etc.) — this is a **DPC Section 22 trigger**. Add a new processing-activity row to this file covering all seven Article 30(1)(a)–(g) headings, and complete the full five-step checklist in the README Per-Client Compliance Setup, Section F (third-party-widget trigger checklist) ("Section 22 third-party-widget trigger checklist") **before** the widget ships to production. The five steps are: written assessment → Article 26 joint-controller arrangement → privacy policy disclosure → consent gating → this RoPA update. Do not skip any — see `legal-compliance/cookies/dpc-guidance-walkthrough.md` Section 22 for the reasoning.
