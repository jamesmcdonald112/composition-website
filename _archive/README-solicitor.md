1. Change all mary molloy references
2. change og image
3.  change favicon
4. create a change logo
5. reword accessibily statement
6. reword privacy policy
7. reword cookie policy
8. check this file - src/pages/robots.txt.ts
...
The testimonials section fetches reviews from the Google Places API (New) and caches them server-side so the API is only called once per day regardless of traffic. (Make sure to make a Google Business Profile).
...
**Status:** Implemented in `vercel.json` at the project root — six baseline headers plus a minimal CSP. The CSP currently allows same-origin scripts, inline styles, local/data fonts, data/HTTPS images, YouTube embeds, same-origin network/form requests, and blocks framing via both `X-Frame-Options` and `frame-ancestors`.

Browsers support a set of HTTP response headers that protect users from common attacks. Vercel provides HTTPS automatically, but it does not replace a project-specific security-header policy.
...
Forces the browser to always use HTTPS for your domain, even if someone types `http://`. After the first visit, the browser will refuse to connect over plain HTTP. This only works after the site is serving correctly over HTTPS.
...
Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; frame-src https://www.youtube.com https://www.youtube-nocookie.com; connect-src 'self'; form-action 'self'; base-uri 'self'; frame-ancestors 'none'
```
...
Before adding new third-party scripts or embeds, update the matching CSP directive in `vercel.json`. A wrong CSP can silently break videos, forms, scripts, styles, and tracking.
...
- Booking or funnel embeds usually need `script-src`, `frame-src`, `img-src`, and `connect-src`.
- CDN-hosted fonts usually need `style-src` and `font-src`.

After deployment, test the live site with `securityheaders.com`, Lighthouse, and the browser DevTools Network panel.
...
- **Verify Vercel security headers post-deploy** — the six baseline headers (XFO, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, COOP, HSTS) plus a minimal CSP are configured in `vercel.json` at the project root. The CSP currently allows `'self'`, inline styles (required by Astro), data/https images, YouTube embeds (`youtube.com` + `youtube-nocookie.com`), and same-origin connect/form-action. **After deploy:** scan the production URL via [securityheaders.com](https://securityheaders.com) — aim for an A rating. **Before adding new third-party scripts** (Cookiebot, analytics, fonts from a CDN, embedded booking widgets), extend the matching CSP directive in `vercel.json` or those resources will be silently blocked.
